...
- **Docs**: See `deployment/docker-compose/README.md` for environment variables, build, and `docker compose up -d --build` instructions.
...
- **Docs**: See `deployment/virtual-machine/README.md` for server bootstrap (`setup-server.sh`), env setup (`server/.env`, `client/.env`), and service management.
...
- **Docs**: See `deployment/kubernetes/README.md` for image build/push commands, manifests, secrets, and rollout/rollback steps.
