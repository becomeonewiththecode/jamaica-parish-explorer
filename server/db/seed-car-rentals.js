...
try { db.exec('ALTER TABLE places ADD COLUMN description TEXT'); } catch (e) { /* exists */ }
try { db.exec('ALTER TABLE places ADD COLUMN image_url TEXT'); } catch (e) { /* exists */ }

const getParishId = db.prepare('SELECT id FROM parishes WHERE slug = ?');
const insertPlace = db.prepare(`
  INSERT OR IGNORE INTO places (parish_id, osm_id, name, category, lat, lon, address, phone, website, opening_hours, cuisine, stars, description, image_url)
  VALUES (?, ?, ?, 'car_rental', ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?)
`);
...
const carRentals = [
  ...
  { name: 'Avis Rent A Car - Kingston Airport', lat: 17.9360, lon: -76.7868, parish: 'kingston', address: 'Norman Manley International Airport', phone: '+1 876-924-8293', website: 'https://www.avis.com.jm', hours: 'Mo-Su 07:00-23:00' },
  ...
  { name: 'Caribbean Car Rentals - Kingston', lat: 17.9680, lon: -76.7835, parish: 'kingston', address: '33 Trafalgar Road, Kingston 10', phone: '+1 876-906-0583', website: 'https://www.caribbeancarrentals.net', hours: 'Mo-Sa 08:00-17:00' },
  ...
  { name: 'Island Car Rentals - Montego Bay', lat: 18.5035, lon: -77.9140, parish: 'st-james', address: 'Sangster International Airport', phone: '+1 876-952-5771', website: 'https://www.islandcarrentals.com', hours: 'Mo-Su 07:00-22:00' },
  ...
  { name: 'Hertz - Montego Bay Airport', lat: 18.5030, lon: -77.9148, parish: 'st-james', address: 'Sangster International Airport', phone: '+1 876-979-0438', website: 'https://www.hertz.com', hours: 'Mo-Su 07:00-23:00' },
  { name: 'Sixt Rent A Car - Montego Bay', lat: 18.5028, lon: -77.9150, parish: 'st-james', address: 'Sangster International Airport', phone: '+1 876-684-9802', website: 'https://www.sixt.com', hours: 'Mo-Su 08:00-22:00' },
  { name: 'Jamaica Car Rental - Hip Strip', lat: 18.4750, lon: -77.9225, parish: 'st-james', address: 'Gloucester Avenue, Montego Bay', phone: '+1 876-952-5586', website: null, hours: 'Mo-Sa 08:00-18:00' },
  ...
  { name: 'Eastern Car Rentals - Port Antonio', lat: 18.1790, lon: -76.4510, parish: 'portland', address: '16 West Street, Port Antonio', phone: '+1 876-993-3624', website: null, hours: 'Mo-Sa 08:00-17:00' },

  // Mandeville / Manchester
  { name: 'Prospere Car Rentals - Mandeville', lat: 18.0410, lon: -77.5035, parish: 'manchester', address: 'Ward Avenue, Mandeville', phone: '+1 876-962-2245', website: null, hours: 'Mo-Sa 08:00-17:00' },
  ...
  // Ian Fleming Airport / St. Mary
  { name: 'Island Car Rentals - Boscobel', lat: 18.4050, lon: -76.9690, parish: 'st-mary', address: 'Ian Fleming International Airport', phone: '+1 876-975-5100', website: 'https://www.islandcarrentals.com', hours: 'Mo-Su 08:00-18:00' },
];

async function tryBingImage(name) {
  const query = `${name} Jamaica`;
...
async function main() {
  ...
  for (const cr of carRentals) {
    const parish = getParishId.get(cr.parish);
    if (!parish) {
    ...
    try {
      insertPlace.run(
        parish.id, osmId, cr.name, cr.lat, cr.lon,
        cr.address, cr.phone, cr.website, cr.hours,
        '', image || ''
      );
      inserted++;
  ...
  console.log(`\nDone! Inserted ${inserted}. Total car rentals in DB: ${count.c}`);
  db.close();
}
